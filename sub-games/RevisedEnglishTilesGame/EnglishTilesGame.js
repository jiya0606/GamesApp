...
import LetterComponent from '/Users/jiya/Games/sub-games/RevisedEnglishTilesGame/LetterComponent.js';
import DropTiles from '/Users/jiya/Games/sub-games/RevisedEnglishTilesGame/DropTiles.js';
...
global.cardsFromDB = new Array();
const loadVocabCards = () => {
  return fetch('http://54.69.90.57/api/words')
    .then((response) => response.json())
    .then((json) => {
      // parse the json and prepare it in the format you need it      
      for (var x = 0; x < json.words.length; x++) {
        cardsFromDB.push(json.words[x])
        cardsFromDB[x].letters = json.words[x].englishText.split('');
      }
